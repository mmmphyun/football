...
def verify_match(match_id: int, match_label: str, target_team_name: str) -> dict[str, Any]:
    ...
    print(f"  Starting Formation: {form['formation']}, Length: {form['team_length']}m, Width: {form['team_width']}m")
    print("  Subphase Shapes:")
    for sp_k, sp_v in form["subphases"].items():
        print(f"    - {sp_k:12s} ({sp_v['name']}): shape={sp_v['formation']:7s}, line_height={sp_v['line_height']:5.2f}m, length={sp_v['length']:5.2f}m")

    print("\n  Starters Tactical Roles & Empirical Coordinates:")
    for p in form["starters"]:
        print(f"    #{p['jersey_number']:<2d} {p['player_name']:<24s} | Pos: {p['position']:<22s} | Role: {p['tactical_role']:<26s} ({p['tactical_role_ko']}) | Coords: ({p['x']:5.2f}, {p['y']:5.2f}) [Events: {p['event_count']}]")

    # 2. 5대 시그니처 공격 플레이북
    playbook = compute_playbook_summary(events, target_team_id)
    print(f"\n[2. PLAYBOOK SIGNATURE PATTERNS] Found {len(playbook)} patterns:")
    for pat in playbook:
        print(f"  * {pat['name']} ({pat['name_ko']}): {pat['occurrences']} occurrences, Total xG: {pat['total_xg']}, Sequences Stored: {len(pat['sequences'])}")
        if pat["sequences"]:
            seq = pat["sequences"][0]
            print(f"    Sample ({len(seq)} steps): " + " -> ".join(f"{s['type']} by {s['player_name']}" for s in seq))
    ...
    for trap in pressure["pressure_traps"]:
        print(f"    - {trap['zone']}: {trap['count']} times at ({trap['x']}, {trap['y']}) [Intensity: {trap['intensity']}]")
