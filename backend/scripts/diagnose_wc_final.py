...
def run_wc_final_diagnosis() -> None:
    ...
    wc = next(c for c in comps if c.get("competition_name") == "FIFA World Cup" and c.get("season_name") == "2022")
    matches = dl.fetch_matches(wc["competition_id"], wc["season_id"])
    final_m = next(m for m in matches if ("Argentina" in m.get("home_team", {}).get("home_team_name", "") or "Argentina" in m.get("away_team", {}).get("away_team_name", "")) and ("France" in m.get("home_team", {}).get("home_team_name", "") or "France" in m.get("away_team", {}).get("away_team_name", "")))
    match_id = final_m["match_id"]
    print(f"2022 World Cup Final Match ID: {match_id} ({final_m.get('home_team', {}).get('home_team_name')} vs {final_m.get('away_team', {}).get('away_team_name')})")
    ...
    # 팀 매핑
    arg_id = next(ev["team"]["id"] for ev in events if ev.get("team") and "Argentina" in ev["team"]["name"])
    fra_id = next(ev["team"]["id"] for ev in events if ev.get("team") and "France" in ev["team"]["name"])

    print("=" * 80)
    print(f"MATCH {match_id} (Argentina vs France) TACTICAL DATA DIAGNOSIS")
    print("=" * 80)
    print(f"Total Events: {len(events)} (Argentina: {sum(1 for ev in events if ev.get('team', {}).get('id') == arg_id)}, France: {sum(1 for ev in events if ev.get('team', {}).get('id') == fra_id)})")
    ...
    for pid in list(arg_starter_ids):
        ...
        print(f"    {p_name:<28s} ({pos_name:<22s}) | Raw: ({raw_in_x:5.1f}, {raw_in_y:5.1f}) [n={len(raw_in):3d}] | Curr Output: ({curr_p.get('x', 0):5.1f}, {curr_p.get('y', 0):5.1f}) | Anchor: ({curr_p.get('anchor_x'):5.1f}, {curr_p.get('anchor_y'):5.1f})")
    ...
    arg_pressures = [ev for ev in events if ev.get("team", {}).get("id") == arg_id and ev.get("type", {}).get("name") == "Pressure"]
    fra_pressures = [ev for ev in events if ev.get("team", {}).get("id") == fra_id and ev.get("type", {}).get("name") == "Pressure"]

    fra_passes_def_40 = [ev for ev in events if ev.get("team", {}).get("id") == fra_id and ev.get("type", {}).get("name") == "Pass" and (ev.get("location") and ev.get("location")[0] <= 48.0)]
    arg_def_actions_att_60 = [ev for ev in events if ev.get("team", {}).get("id") == arg_id and ev.get("type", {}).get("name") in {"Pressure", "Tackle", "Interception", "Foul Committed"} and (ev.get("location") and ev.get("location")[0] >= 48.0)]
